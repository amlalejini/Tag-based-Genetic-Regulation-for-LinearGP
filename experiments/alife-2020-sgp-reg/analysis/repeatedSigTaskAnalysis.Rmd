...
data_loc <- "../data/alt-sig/max_fit_orgs.csv"
...
4-state environment; run id = 234; similarity & regulator states over time
...
trace_file <- paste(data_dir, "traces-1/trace_update-10000_run-id-", trace_id, ".csv", sep="")
...
reg_plot <- 
  ...
  geom_tile(data=filter(trace_data, is_running==TRUE | triggered==TRUE), color="black", width=1, height=1, size=0.8) +
...
match_plot <- 
  ggplot(trace_data, aes(x=mod_id_x_pos, y=time_step, fill=similarity_score)) +
...
```{r}
# Networks!
graph_nodes_loc <- paste("../data/alt-sig/reg-igraphs/reg_graph_", trace_id, "_nodes.csv", sep="")
graph_edges_loc <- paste("../data/alt-sig/reg-igraphs/reg_graph_", trace_id, "_edges.csv", sep="")
...
E(network)$frame
network_out_name <- paste("./imgs/case-study-id-", trace_id, "-network.svg", sep="")
...
library(magick)
library(svglite)
network_img <- ggdraw() + draw_image(network_out_name)

grid_out_name <- paste("./imgs/case-study-id-", trace_id, ".svg", sep="")
grid <- plot_grid(reg_plot, match_plot, 
                  labels=c("A", "B"), ncol=1)
grid
save_plot(grid_out_name, grid)
