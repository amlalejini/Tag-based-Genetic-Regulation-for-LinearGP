...
ggplot(sol_data, aes(x=condition, fill=condition)) +
  ...
  scale_y_continuous(name="Number of evolved solutions", breaks=seq(0, 50, 10), limits=c(0, 52)) +
  scale_fill_discrete(name="SignalGP Treatment",
                      limits=c("regulation", "memory", "none", "both"),
                      labels=c("Regulation-only", "Memory-only", "Neither", "Regulation & Memory")) +
...
ggplot(filter(data, solution=="1"), aes(x=condition, y=update, color=condition)) +
  ...
  scale_x_discrete(breaks=c("regulation", "memory", "none", "both"),
                   labels=c("Regulation", "Global\nMemory", "Neither", "Both")) +
  facet_wrap(~ NUM_SIGNAL_RESPONSES, scales="free_y", labeller=labeller(NUM_SIGNAL_RESPONSES=label_lu)) +
  ggsave("./imgs/repeated-signal-solve-time.png", width=16,height=8)
```
...
reg_and_mem_solutions$strategy <- mapply(get_strategy, reg_and_mem_solutions$use_regulation, reg_and_mem_solutions$use_global_memory)
...
ggplot(reg_and_mem_solutions, mapping=aes(x=NUM_SIGNAL_RESPONSES, fill=strategy)) +
  ...
  scale_fill_discrete(breaks=c("use regulation", "use memory", "use neigther", "use both"),
