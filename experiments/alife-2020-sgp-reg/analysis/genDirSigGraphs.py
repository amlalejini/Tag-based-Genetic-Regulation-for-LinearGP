...
def main():
    ...
    test_ids = [i for i in range(0, 16)]
    ...
    for graph_file in graphs:
        ...
        for test_id in test_ids:
            ...
            for line in lines:
                ...
                if info["test_id"] != str(test_id): continue
                ...
                # reg_deltas = list(map(float, info["match_deltas"].strip("[]").split(",")))
                # Update nodes
            ...
            if dump_igraphs:
                ...
                nodes_out_path = os.path.join(dump_dir, f"reg_graph_id-{run_id}_env-{test_id}_nodes.csv")
                edges_out_path = os.path.join(dump_dir, f"reg_graph_id-{run_id}_env-{test_id}_edges.csv")
            ...
            graph_info["reciprocity"] = nx.overall_reciprocity(graph)
