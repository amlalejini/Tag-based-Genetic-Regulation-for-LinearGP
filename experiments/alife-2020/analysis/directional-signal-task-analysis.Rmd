...
Specifically, we added a second environmental signal taht shifts the required response backward to the 'previous' response.
...
We intend to use this task to explore whether a more challenging task (the directional signal task) facilitates promotes the evolution of more complex regulatory networks than a simpler task (the repeated signal task).
...
We see that the majority of replicates (`r nrow(filter(data, solution==1)) / replicates`) yielded a program capable of perfectly solving the directional signal task.
...
# Plot!
out_name <- 
  paste("./imgs/case-study-trace-id-", trace_id, "-env-id-", env_seq_id, "-regulator-state-horizontal.pdf", sep="")
reg_state_plot <- 
  ...
  ggsave(out_name, width=4, height=2.75)
reg_state_plot
```
...
# this ordering gives us minimal edge crossing...
...
draw_network(network, TRUE, network_out_name, vertex_order)
