...
def main():
    ...
    parser.add_argument("--update", type=int, help="At what update should we pull max fit orgs from? -1 for max in file", default=-1)
    args = parser.parse_args()
    data_dirs = args.data
    dump_dir = args.dump
    dump_fname = args.out_fname
    pull_update = args.update
    ...
    for run in run_dirs:
        ...
        best_org_id = -1
        if pull_update < 0:
            # Pull highest fitness organism
            best_org_id = 0
            for i in range(len(orgs)):
                if float(orgs[i][header_lu["score"]]) > float(orgs[best_org_id][header_lu["score"]]):
                    best_org_id = i
        else:
            # Pull at requested update (or closest possible but not exceeding)
            best_org_id = 0
            for i in range(len(orgs)):
                best_org_id = i
