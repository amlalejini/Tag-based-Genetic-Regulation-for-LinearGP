---
title: "Repeated Signal Task Analysis"
output:
  html_document:
    keep_md: no
    toc: true
...
Here, we give an overview of the repeated signal diagnostic task (mirroring the overview given in the paper), and we provide data analyses for our repeated signal task experiments. All source code for our statistical analyses and data visualizations is embedded in this document. 
...
```{r}
max_fit_org_data_loc <- "data/max_fit_orgs_noprogram.csv"
...
# Load network data
reg_network_data <- read.csv("data/reg_graphs_summary.csv", na.strings="NA")

# Load instruction execution data
inst_exec_data <- read.csv("data/exec_trace_summary.csv", na.strings="NA")
...
Further, as the repeated signal task increases in difficulty (i.e., the number of repeated signals increases), we expected the success differential to increase.
...
ggplot(sol_data, aes(x=condition, fill=condition)) +
  ...
  scale_fill_discrete(name="Condition:",
                      limits=c("regulation", "memory", "both"),
                      labels=c("Regulation-only (R)", "Memory-only (M)", "Both (B)")) +
...
We confirmed that it is possible to solve the repeated signal task at each problem difficulty with [hand-coded programs](../../../handcoded-examples.md).
However, we see that many more replicates yield solutions when given access to genetic regulation.
...
These data imply that genetic regulation is more readily adaptable than global memory usage (as it is currently designed/implemented) for the repeated signal task.
...
ggplot(max_fit_org_data, aes(x=condition, y=score, color=condition)) +
  ...
  scale_color_discrete(name="Condition",
                       breaks=c("regulation", "memory", "both"),
                       labels=c("Regulation-only (R)", "Global-memory-only (M)", "Both (B)")) +
  scale_x_discrete(name="Condition",
                   breaks=c("regulation", "memory", "both"),
                   labels=c("R", "M", "B")) +
  facet_wrap(~ NUM_SIGNAL_RESPONSES,
...
Here, we compare the generation at which solutions arise. Note that it is not a truly fair comparison to make between conditions where an different numbers of replicates produce solutions.
...
ggplot(sol_data,
  ...
  scale_color_discrete(name="Condition: ",
                       breaks=c("regulation", "memory", "both"),
                       labels=c("Regulation (R)", "Global memory (M)", "Both (B)")) +
  scale_x_discrete(breaks=c("regulation", "memory", "both"),
                   labels=c("R", "M", "B")) +
...
for (env in env_complexities) {
  env_donut_data  <- data.frame(
    ...
    count=c(
      nrow(filter(reg_and_mem_solutions, condition=="both" & NUM_SIGNAL_RESPONSES==env & strategy=="use neither")),
      nrow(filter(reg_and_mem_solutions, condition=="both" & NUM_SIGNAL_RESPONSES==env & strategy=="use memory")),
      nrow(filter(reg_and_mem_solutions, condition=="both" & NUM_SIGNAL_RESPONSES==env & strategy=="use regulation")),
      nrow(filter(reg_and_mem_solutions, condition=="both" & NUM_SIGNAL_RESPONSES==env & strategy=="use both"))
    ),
    category=c("use neither", "use memory", "use regulation", "use both")
  )
  ...
  env_donut_data$label <- paste0(env_donut_data$count, " ", env_donut_data$category)
...
ggplot(donut_data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=category)) +
  ...
  facet_wrap(~env) +
  ggsave("imgs/donut-test.pdf", width=10, height=10)
```
...
### What distribution of instructions do evolved digital organsims execute?

```{r}
# only want solutions
solutions_inst_exec_data <- filter(inst_exec_data, SEED %in% sol_data$SEED)

ggplot(solutions_inst_exec_data,
       aes(x=condition, y=total_execution_time, color=condition)) +
  geom_boxplot() +
  scale_color_discrete(name="Condition: ",
                       breaks=c("regulation", "memory", "both"),
                       labels=c("Regulation (R)", "Global memory (M)", "Both (B)")) +
...
ggplot(melted,
       aes(x=inst_type, y=inst_type_prop, color=condition)) +
  geom_boxplot() +
  scale_color_discrete(name="Condition: ",
                       breaks=c("regulation", "memory", "both"),
                       labels=c("Regulation (R)", "Global memory (M)", "Both (B)")) +
...
ggplot(solutions_inst_exec_data,
       aes(x=condition, y=regulation_inst_cnt, color=condition)) +
  geom_boxplot() +
  scale_color_discrete(name="Condition: ",
                       breaks=c("regulation", "memory", "both"),
                       labels=c("Regulation (R)", "Global memory (M)", "Both (B)")) +
...
NOTE: do I actually want the proportion of non-nop instructions?
...
ggplot(melted, aes(x=NUM_SIGNAL_RESPONSES, y=reg_edges_cnt, color=reg_edge_type)) +
  ...
  ggsave("./imgs/repeated-signal-regulation-edge-types.png", width=4, height=3)
...
# reg_network_data$NUM_SIGNAL_RESPONSES <- factor(melted$NUM_SIGNAL_RESPONSES)
ggplot(filter(reg_network_data, run_id %in% temp$SEED ), aes(x=NUM_SIGNAL_RESPONSES, y=edge_cnt)) +
  geom_boxplot() +
  xlab("Environmental Complexity") +
  ylab("# Edges") +
  theme(legend.position="bottom",
...
trace_file <- paste("data/traces/trace_update-10000_run-id-", trace_id, ".csv", sep="")
...
```{r, message=FALSE}
# Networks!
graph_nodes_loc <- paste("data/igraphs/reg_graph_id-", trace_id, "_nodes.csv", sep="")
graph_edges_loc <- paste("data/igraphs/reg_graph_id-", trace_id, "_edges.csv", sep="")
