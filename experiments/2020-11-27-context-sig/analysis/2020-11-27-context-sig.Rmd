...
ggplot(filter(sol_data, task=="S4"), aes(x=condition, fill=condition)) +
  ...
  ggsave(
    paste0(working_directory, "context-signal-solution-counts.pdf"),
    width=4,
    height=4
  )
```
...
reg_disabled_fail_cnt <- nrow(filter(sol_data, task=="S4" & condition=="memory")) - reg_disabled_success_cnt

reg_enabled_success_cnt <- nrow(filter(sol_data, task=="S4" & solution=="1" & condition=="both"))
reg_enabled_fail_cnt <- nrow(filter(sol_data, task=="S4" & condition=="both")) - reg_enabled_success_cnt
...
## Case study: visualizing an evolved gene regulatory network
